...
export class FirestorePostService {
  ...
  static getInstance(): FirestorePostService {
    if (!FirestorePostService.instance) {
      FirestorePostService.instance = new FirestorePostService();
    }
    return FirestorePostService.instance;
  }
  ...
  async createPost(userId: number, data: CreatePostDTO, files?: Express.Multer.File[]): Promise<PostResponseDTO> {
    ...
    const post = await FirestorePostRepository.create({
      authorId: userId,
  ...
  async getPost(postId: number, userId?: number): Promise<PostResponseDTO> {
    const post = await FirestorePostRepository.findById(postId, userId);
  ...
  async getUserFeed(userId: number, page: number = 1, limit: number = 10) {
    const feed = await FirestorePostRepository.getUserFeed(userId, page, limit);
  ...
  async updatePost(userId: number, postId: number, data: UpdatePostDTO): Promise<PostResponseDTO> {
    ...
    const updatedPost = await FirestorePostRepository.update(postId, data);

    return this.formatPostResponse(updatedPost, userId);
  }

  async deletePost(userId: number, postId: number): Promise<{ message: string }> {
    const post = await FirestorePostRepository.findByIdSimple(postId);
    ...
    const mediaUrls = await FirestorePostRepository.deleteWithMedia(postId);
  ...
  async archivePost(userId: number, postId: number): Promise<{ message: string }> {
    ...
    await FirestorePostRepository.archive(postId);
...
export default FirestorePostService.getInstance();
