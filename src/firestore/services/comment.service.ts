...
export class FirestoreCommentService {
  ...
  static getInstance(): FirestoreCommentService {
    if (!FirestoreCommentService.instance) {
      FirestoreCommentService.instance = new FirestoreCommentService();
    }
    return FirestoreCommentService.instance;
  }
  ...
  async addComment(
    ...
    const comment = await FirestoreCommentRepository.create({
      postId,
      userId,
  ...
  async getPostComments(postId: number, userId?: number): Promise<CommentResponseDTO[]> {
    ...
    const comments = await FirestoreCommentRepository.findByPostId(postId, userId);
  ...
  async getComment(commentId: number, userId?: number): Promise<CommentResponseDTO> {
    ...
    const comment = await FirestoreCommentRepository.findById(commentId, userId);
  ...
  async updateComment(
    ...
    logger.info(`Updating comment ${commentId} by user ${userId}`, { commentId, userId });
    ...
    const updatedComment = await FirestoreCommentRepository.update(commentId, data.content, userId);
  ...
  async deleteComment(userId: number, commentId: number): Promise<{ message: string }> {
    ...
    await FirestoreCommentRepository.delete(commentId, userId);
  ...
  async likeComment(userId: number, commentId: number): Promise<{ liked: boolean; likesCount: number }> {
    ...
    const { liked } = await FirestoreLikeRepository.toggleCommentLike(userId, commentId);
    
    const comment = await FirestoreCommentRepository.findById(commentId);
...
export default FirestoreCommentService.getInstance();
